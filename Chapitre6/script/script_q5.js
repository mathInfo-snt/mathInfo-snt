prop1 = ['"1" />raster','"0" />vector','"0" />matrice','"0" />matrix',
        '"0" />cm','"0" />map'];
prop2 = ['"1" />Google Maps','"0" />Geoportail' ,'"0" />Mappy','"0" />OpenStreetMap',
        '"0" />Google Earth','"0" />Georando'];
prop3 = ['"1" />vectorielles','"0" />matricielles','"0" />anti-pixel','"0" />raster','"0" />no-pixel',
            '"0" />pixel'];
...
function verification(nb)
{
    ...
    if (somme==nb)
    {
        afrep.innerHTML="<span class='reponse_juste'>"+"Toutes les réponses sont correctes ! Vous pouvez passer à la suite : " 
                +"<a href=gl_exercice5.1.html target='_blank'><b class='abas'>Exercice 5.1</b></a>";
        if(typeof localStorage!='undefined') 
        {
            localStorage.setItem('c6p5q5',1);
            fait();
        }
    }
    else
