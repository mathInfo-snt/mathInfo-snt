prop1 = ['"1" />Propriétés','"0" />Options','"0" />Outils','"0" />Données',
         '"0" />Métadonnées','"0" />Général','"0" />Format','"0" />Applications'];
prop2 = ['"1" />Outils','"0" />Options','"0" />Propriétés','"0" />Format','"0" />Style','"0" />Données','"0" />Métadonnées','"0" />Général'];
...
function verification(nb)
{
    ...
    if (somme==nb)
    {
        afrep.innerHTML="<span class='reponse_juste'>"+"Toutes les réponses sont correctes ! Vous pouvez passer à la suite : " 
                +"<a href=don_exercice6.1.html target='_blank'><b class='abas'>Exercice 6.1</b></a>";
        if(typeof localStorage!='undefined') 
        {
            localStorage.setItem('c4p6q6',1);
            fait();
        }
    }
    else
