prop1 = ['"1" />Chrome','"0" />Google','"0" />Bing','"0" />Yahoo','"0" />Qwant','"0" />Duckduckgo'];
prop2 = ['"1" />spiders','"0" />routeurs','"0" />chercheurs','"0" />moteurs','"0" />motors','"0" />scanners'];
prop3 = ['"1" />Duckduckgo','"0" />Firefox','"0" />Safari','"0" />Windows','"0" />Ubuntu','"0" />Mozilla'];
...
function verification(nb)
{
    ...
    if (somme==nb)
    {
        afrep.innerHTML="<span class='reponse_juste'>"+"Toutes les réponses sont correctes ! Vous pouvez passer à la suite : " 
                +"<a href=web_exercice5.1.html target='_blank'><b class='abas'>Exercice 5.1</b></a>";
        if(typeof localStorage!='undefined') 
        {
            localStorage.setItem('c1p5q5',1);
            fait();
        }
    }
    else
